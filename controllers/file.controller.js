    ...
    try {
        ...
        if (!allowedFileTypes.includes(req.file.mimetype)) {
            ...
            const stream = fs.createReadStream(filePath)
                ...
                .on('error', (error) => {
                    responseStatus(res, 400, 'Invalid CSV file format', null, true);
                })
                .on('data', (row) => {
                    csvData.push(row);
                })
                    ...
                    try {
                        ...
                        for (const row of csvData) {
                            ...
                            const queryCheckExisting =
                                'SELECT * FROM merchants WHERE merchant_name = $1 AND merchant_description = $2 AND merchant_city = $3 AND merchant_postalcode = $4';
                        ...
                        const queryInvalidPostalCode =
                            "SELECT * FROM merchants WHERE merchant_postalcode !~ '^[0-9]{5}$'";
                        ...
                        if (invalidPostalCode.length > 0) {
                            const queryDeleteInvalidPostalCode =
                                "DELETE FROM merchants WHERE merchant_postalcode !~ '^[0-9]{5}$'";
